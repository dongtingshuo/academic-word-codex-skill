...
以下页面来自同一份端到端验收报告，是 Skill 实际生成、通过结构验证，并由 Microsoft Word 原生导出的 Word 成品，不是设计示意图。
...
封面不是固定的，使用 Skill 时可以直接说明希望采用哪一种：

- **通用课程报告封面（默认）**：填写自己的学校或单位、课程、姓名、学号、教师和日期，不出现华科名称或学位声明。
- **学校或课程自定义封面**：提供学校、学院或任课教师发放的 Word 模板，Skill 会保留其封面和前置结构，并在确认格式冲突后套用正文样式。
- **华科论文封面**：只有明确要求 `hust_thesis` 时启用，同时生成中文封面、英文题名页和声明，并要求用户提供全部真实信息。

例如可以说：“使用通用课程报告封面”“使用我上传的学校模板作为封面”或“使用华科硕士论文封面”。如果没有说明，始终使用通用封面。
